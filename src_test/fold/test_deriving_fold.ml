...
type 'a pbtree = [`Node of 'a pbtree * 'a * 'a pbtree | `Leaf ]
[@@deriving fold]

let test_pbtree ctxt =
  let pbtree  = (`Node (`Node (`Leaf, 3, `Leaf), 1, `Node (`Leaf, 2, `Leaf))) in
  assert_equal ~printer:string_of_int 6 (fold_pbtree (+) 0 pbtree)
