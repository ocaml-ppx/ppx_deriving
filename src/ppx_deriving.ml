...
let args_spec = [
  ("-deriving-plugin",
   A.String (fun str -> plugins_to_load := str :: !plugins_to_load),
...
let rewriter config cookies =
  ...
  let structure mapper = function
    | [%stri [@@@findlib.ppxopt [%e? { pexp_desc = Pexp_tuple (
      ...
      let anon_fun arg =
        raise (A.Bad ("Unexpected argument in cookie: " ^ arg)) in
      A.parse_argv ~current:(ref 0) args args_spec anon_fun "";
      add_plugins cookies !plugins_to_load;
      plugins_to_load := [];
      mapper.Ast_mapper.structure mapper rest
    | items -> mapper.Ast_mapper.structure mapper items in
  { mapper with Ast_mapper.structure }
